...
bool Config::getConfig(const char *FileName)
{
    ...
    element = algorithm->FirstChildElement(CNS_TAG_PR);
    if (!element) {
        std::cout << "Warning! No '" << CNS_TAG_PR << "' tag found in XML file" << std::endl;
        std::cout << "Value of '" << CNS_TAG_PR << "' was defined to 'random'" << std::endl;
        SearchParams[CN_SP_PR] = CN_SP_PR_RAND;
    }
    else {
        value = element->GetText();
        std::transform(value.begin(), value.end(), value.begin(), ::tolower);
        if (value == CNS_SP_PR_RAND) SearchParams[CN_SP_PR] = CN_SP_PR_RAND;
        else if (value == CNS_SP_PR_SHORT) SearchParams[CN_SP_PR] = CN_SP_PR_SHORT;
        else if (value == CNS_SP_PR_LONG) SearchParams[CN_SP_PR] = CN_SP_PR_LONG;
        else if (value == CNS_SP_PR_CUSTOM) SearchParams[CN_SP_PR] = CN_SP_PR_CUSTOM;
        else {
            std::cout << "Warning! Value of '" << CNS_TAG_PR << "' is not correctly specified." << std::endl;
            std::cout << "Value of '" << CNS_TAG_PR << "' was defined to 'random'" << std::endl;
            SearchParams[CN_SP_PR] = CN_SP_PR_RAND;
        }
    }
