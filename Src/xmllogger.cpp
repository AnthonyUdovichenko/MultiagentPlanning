...
void XmlLogger::writeAgent(const char *FileName, int agent_number) {
    XMLElement *log = doc.FirstChildElement(CNS_TAG_ROOT);
    log = log->FirstChildElement(CNS_TAG_LOG);

    XMLElement *agent = doc.NewElement(CNS_TAG_AGENT);
    agent->SetAttribute(CNS_TAG_ATTR_NUM, agent_number + 1);

    if (loglevel != CN_LP_LEVEL_NOPE_WORD) {
        XMLElement *element = doc.NewElement(CNS_TAG_MAPFN);
        element->LinkEndChild(doc.NewText(FileName));
        agent->InsertEndChild(element);

        agent->InsertEndChild(doc.NewElement(CNS_TAG_SUM));

        agent->InsertEndChild(doc.NewElement(CNS_TAG_PATH));

        agent->InsertEndChild(doc.NewElement(CNS_TAG_LPLEVEL));

        agent->InsertEndChild(doc.NewElement(CNS_TAG_HPLEVEL));
    }

    if (loglevel == CN_LP_LEVEL_FULL_WORD || loglevel == CN_LP_LEVEL_MEDIUM_WORD)
        agent->InsertEndChild(doc.NewElement(CNS_TAG_LOWLEVEL));
...
void XmlLogger::writeToLogMap(const Map &map, const std::list<Node> &path)
{
    ...
    mapTag = mapTag->FirstChildElement(CNS_TAG_LOG)->LastChildElement()->FirstChildElement(CNS_TAG_PATH);
...
void XmlLogger::writeToLogPath(const std::list<Node> &path)
{
    ...
    lplevel = lplevel->FirstChildElement(CNS_TAG_LOG)->LastChildElement()->FirstChildElement(CNS_TAG_LPLEVEL);
...
void XmlLogger::writeToLogHPpath(const std::list<Node> &hppath)
{
    ...
    hplevel = hplevel->FirstChildElement(CNS_TAG_LOG)->LastChildElement()->FirstChildElement(CNS_TAG_HPLEVEL);
...
void XmlLogger::writeToLogSummary(unsigned int numberofsteps, unsigned int nodescreated, float length, double time, double cellSize)
{
    ...
    summary = summary->FirstChildElement(CNS_TAG_LOG)->LastChildElement()->FirstChildElement(CNS_TAG_SUM);
...
void XmlLogger::writeToLogNotFound()
{
    ...
    XMLElement *node = doc.FirstChildElement(CNS_TAG_ROOT)->FirstChildElement(CNS_TAG_LOG)->LastChildElement()->FirstChildElement(CNS_TAG_PATH);
